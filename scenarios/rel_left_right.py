...
class LeftRight(BaseScenario):
    ...
    def observation(self, agent: Agent):
        ...
        # will only work if two agents
        obs.extend([a.state.pos - agent.state.pos for a in self.world.agents if a is not agent])
        obs.extend([a.state.vel - agent.state.vel for a in self.world.agents if a is not agent])
